...
Student::Student(std::string id, std::string name, std::string pass)
    : Person(std::move(id), std::move(name), std::move(pass)),
      personalTimetable(new Timetable()) { // Composition: Student owns its Timetable
}

Student::~Student() {
    delete personalTimetable; // Clean up owned resource
...
void Student::showMenu() {
    ...
    std::cout << " 6. View Attendance Percentage\n";
    std::cout << " 0. Logout\n";
    std::cout << "========================================\n";
...
void Student::addCourse(Course* c) {
    ...
    if (it == enrolledCourses.end()) {
        ...
        // Add the course slots to the student's personal timetable
...
void Student::removeCourse(Course* c) {
    ...
    if (it != enrolledCourses.end()) {
        ...
        // Rebuild personal timetable from remaining enrolled courses
