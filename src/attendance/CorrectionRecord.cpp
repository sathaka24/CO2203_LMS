...
CorrectionRecord::CorrectionRecord(std::string sID, std::string lecturerID, std::string reasonStr, std::string status, std::string time)
    : studentID(std::move(sID)), actingLecturerID(std::move(lecturerID)), 
      reason(std::move(reasonStr)), status(std::move(status)),  timestamp(std::move(time)) {}
...
std::string CorrectionRecord::getStatus() const {return status;}